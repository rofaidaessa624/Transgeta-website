 ...
 const slides: SlideItem[] = [
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        const academicOnly = allServices.filter((s) => s.category === "academic");
        setServices(academicOnly);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const openWhatsApp = (serviceName: string) => {
    const message = isArabic
      ? `مرحباً، أريد طلب خدمة أكاديمية: ${serviceName}`
      : `Hello, I want to order this academic service: ${serviceName}`;
